...
{
    strncpy(drvinfo->driver, "linux_ngknet", sizeof(drvinfo->driver));
...
ngknet_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case NGKNET_DEV_INIT:
        ...
        if (dev->flags & NGKNET_DEV_ACTIVE) {
            ...
            strncpy(dev_cfg->name, pdev->name, sizeof(dev_cfg->name));
        ...
        strncpy(pdev->name, dev_cfg->name, sizeof(pdev->name));
        ...
        for (dt = 0; dt < drv_num; dt++) {
            ...
            if (!strcasecmp(dev_cfg->type_str, drv_ops[dt]->drv_desc)) {
                pdev->dev_type = dt;
                strncpy(dev->var_str, dev_cfg->var_str, sizeof(dev->var_str));
                break;
            }
        }
